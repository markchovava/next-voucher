    ...
    <>
        ...
        <section className='w-[100%] h-auto pb-[3rem]'>
            <section className='mx-auto w-[90%] p-[2.5rem] bg-white drop-shadow-lg'>
                ...
                <div className="w-[100%] mb-[2rem] flex items-center justify-start">
                    <label className='lg:w-[20%] w-[40%] gap-3 font-semibold'>Campaign Name:</label>
                    <div className='lg:w-[80%] w-[60%]'>
                        {data.campaign?.name} </div>
                </div>
                <div className="w-[100%] mb-[2rem] flex items-center justify-start">
                    <label className='lg:w-[20%] w-[40%] gap-3 font-semibold'>Code:</label>
                    <div className='lg:w-[80%] w-[60%]'>
                        {data.code} </div>
                </div>
                { data.code &&
                <div className="w-[100%] mb-[2rem] flex lg:flex-row flex-col lg:items-center items-start justify-start">
                    <label className='w-[20%] gap-3 font-semibold'>QR Code:</label>
                    <div className='w-[80%]'>
                        <div
                            ref={qrCodeRef} 
                            className="hidden lg:w-[600px] p-[50px] lg:flex items-center justify-center bg-white drop-shadow-xl">
                            <QRCode value={data.code} size={500} />
                        </div>
                        <div
                            ref={qrCodeRef} 
                            className="lg:hidden w-[450px] p-[50px] flex items-center justify-center bg-white drop-shadow-xl">
                            <QRCode value={data.code} size={400} />
                        </div>
