...
export default function CampaignList() {
    ...
    async function searchData() {
      if(search == ''){
          ...
          try{
              const result = await axiosClientAPI.get(`campaign?search=${search}`, config)
              .then((response) => {
                  setData(response.data.data)
    ...
    async function deleteData(id) {
      try{
        const result = await axiosClientAPI.delete(`campaign/${id}`, config)
        .then((response) => {
          getData()
        })
      } catch (error) {
