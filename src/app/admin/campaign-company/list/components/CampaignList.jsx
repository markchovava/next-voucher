...
export default function CampaignList() {
    ...
    async function searchData() {
      if(search == ''){
          try{
              const result = await axiosClientAPI.get(`campaign-company`, config)
              .then((response) => {
                  setData(response.data.data)
          ...
          try{
              const result = await axiosClientAPI.get(`campaign-company?search=${search}`, config)
              .then((response) => {
                  setData(response.data.data)
    ...
    async function deleteData(id) {
      try{
        const result = await axiosClientAPI.delete(`campaign-company/${id}`, config)
        .then((response) => {
          getData()
        })
      } catch (error) {
      ...
      <>
          ...
          <section className="mx-auto w-[90%] lg:overflow-hidden overflow-auto">
              ...
              <div className="w-[50rem] lg:w-[100%] font-bold flex items-center justify-start bg-slate-100 py-3 border border-slate-200 ">
                    ...
                    <div className="w-[15%] p-3 border-l border-slate-300">START DATE</div>
                    <div className="w-[15%] p-3 border-l border-slate-300">END DATE</div>
              ...
              { data.length > 0 ?
                data.map((item, i) => (
                  <div key={i} className="w-[50rem] lg:w-[100%] flex items-center justify-start py-3 border border-slate-200 ">
                        ...
                        <div className="w-[15%] p-3 border-l border-slate-300">{item.start_date}</div>
                        <div className="w-[15%] p-3 border-l border-slate-300"> {item.end_date}</div>
